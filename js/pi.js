...
(function load() {
	...
	if (isPlugRoom.test(location.href)) {
		...
		function init() {
			...
			var delay = (API.getUser().gRole >= API.ROLE.MANAGER ? 200 : 3500);
			var CHAT_INTERCEPT_STRING = "Plug-It Socket Intercept: " + (Math.random());
			var YTAPIkey = 'AIzaSyC8pk0f57a_UcAIbHdrvRhsmHSG1KZk2SM';
			var url = {
			...
			var session = {
				...
				floodAPI: false,
				woots: API.getUsers().filter(function (x) { return x.vote == 1; }),
				grabs: API.getUsers().filter(function (x) { return x.grab; }),
				mehs:  API.getUsers().filter(function (x) { return x.vote == -1; }),
				chat: []
			};
			...
			function meldBefore(object, property, cb) {
				var originalFunction = object[property];
			...
			function sudo(cb) {
				var originalUser = modules.user.toJSON();
				modules.user.set({
					id: 1,
					guest: false,
					level: 50,
				...
				var originalRoom = _.pick(modules.room.toJSON(), 'joined', 'minChatLevel');
				modules.room.set({
					joined: true,
					minChatLevel: 0,
				}, { silent: true });
			...
			if (version.indexOf('-')) {
				var assign;
				(assign = version.split('-'), _v = assign[0], pre = assign[1]);
			}
			...
			window.pi = {
				...
				_event: {
					...
					chat: function(msg) {
						...
						*/
						...
						var a = msg.type === 'mention' && API.hasPermission(msg.uid, API.ROLE.BOUNCER);
						var b = msg.message.indexOf('@') < 0 && (API.hasPermission(msg.uid, API.ROLE.MANAGER) || ['Dev', 'Community Manager'].indexOf(pi._tool.getRank(sender.id)) !== -1);
						if (a || b) {
					...
					chatCommand: function(cmd) {
						...
						switch (cmd) {
							case 'r':
								var lastMention = $('.cm.mention:last');

								if (lastMention.length !== 0) {
									var user$1 = API.getUser(lastMention[0].getAttribute('data-cid').split('-')[0]);
									API.sendChat('@'+user$1.username +' '+msg);
								} else {
							...
							case 'pm':
								...
								var encoded = '';
								msg.slice(args[0].length+1).split('').forEach(function (e,i,a) {
							...
							case 'exportchat':
								...
								session.chat.forEach(function (e,i,a) {
									file += (csv ?
							...
							case 'ban':
								...
								var duration;
								if (args.length > 1) {
									var lastArg = args[args.length-1].toLowerCase();
							...
							case 'mute':
								...
								var duration;
								if (args.length > 1) {
									var lastArg$1 = args[args.length-1].toLowerCase();
									if (lastArg$1 === 'long' || lastArg$1 === 'medium' || lastArg$1 === 'short')
										{ duration = args.pop(); }
							...
							case 'unrank':
								...
								var roleID = -1; // invalid on purpose
								switch (cmd) {
													...
													if (i + 1 >= a.length && j + 1 >= aa.length) {
														...
														if (toRename.length > 0) {
															toRename.forEach(function(item, index, array) {
																setTimeout(function() {
																	var cleanAuthor = item.song.author.replace(worstRegexp, '').trim();
																	var cleanTitle = item.song.title.replace(worstRegexp, '').trim();
					...
					earn: function(amount) {
						...
						if (settings.gainNotification) {
							var gain;
							var totalXP = API.getUser().xp + amount.xp;
							var totalPP = API.getUser().pp + amount.pp;
							var minXP = amount.xp >= settings.minimumGain.xp;
							var minPP = amount.pp >= settings.minimumGain.pp;
					...
					scoreUpdate: function(score) {
						...
						session.woots = API.getUsers().filter(function (x) { return x.vote == 1; });
						session.grabs = API.getUsers().filter(function (x) { return x.grab; });
						session.mehs  = API.getUsers().filter(function (x) { return x.vote == -1; });
					},
					userJoin: function(join) {
						// contains: {userObject};
						if (settings.userJoin) {
							var userInfo = (settings.userInfo ? ' Lvl:'+join.level+'|'+'Id:'+join.id : '');
							pi._tool.log(
					...
					userLeave: function(leave) {
						...
						if (settings.userLeave) {
							var userInfo = (settings.userInfo ? ' Lvl:'+leave.level+'|'+'Id:'+leave.id : '');
							pi._tool.log(
				...
				_modulesEvent: {
					chatReceive: function(msg) {
						...
						if ($(selector).length) {
							// Script ranks
							pi._tool.getRank(sender.id).forEach(function (rank) {
								switch(rank) {
									case "Dev":
						...
						if (pi._tool.getRank(sender.id).indexOf('Discord Bot') !== -1) {
							...
							if ($(selector).length) {
								...
								var previousMsgs = $('#chat .cm.pi-discord-bot:last .text')[0].innerHTML.split('<br>');
								previousMsgs.pop();
						...
						// Emojis
						var emojiRE = new RegExp(':([a-z0-9-_]+):', 'ig');
						if (pi._tool.getPlugSettings().emoji && emojiRE.test(msg.message)) {
							var match = emojiRE.exec(msg.message);
					...
					chatDelete: function(message) {
						...
						if ($('.cid-'+ message.p.c).length === 0) {
							...
							if (secret[0] == API.getUser().id && secret[1].length !== 13) {
								var text = '';
								secret[1].match(/.{1,3}/g).forEach(function (e,i,a) {
						...
						if (settings.showDeletedMsg) {
							...
							var $chat = $('.cid-'+message.p.c).closest('.cm');
							var time = modules.utils.getSimpleTimestamp();
							var user = API.getUser(message.p.c.split('-')[0]);
							var mod = API.getUser(message.p.mi);
							var $prevDelete = $chat.find('.pi-deleted[data-id="'+message.p.mi+'"]');
							...
							if ($prevDelete.length > 0) {
								var newCount = parseInt($prevDelete.attr('data-count'))+1;
				...
				_DOMEvent: {
					...
					collideRanks: function() {
						var groupDetect = setInterval(function() {
							var group = $('.group');
					...
					keyboardShortcuts: function(e) {
						if (settings.keyboardShortcuts) {
							switch (e.key) {
											...
											success: function(data) {
												pi._tool.getPlaylists(function(playlists) {
													$.ajax({
														...
														url: "/_/grabs",
														data: JSON.stringify({
															playlistID: playlists.filter(function (x) { return x.active; })[0].id,
								...
								case 'G':
									...
									if (!$('#chat-input').hasClass('focused') &&
										...
										pi._tool.getPlaylists(function(playlists) {
											$.ajax({
												type: 'POST',
												url: '/_/playlists/'+playlists.filter(function (x) { return x.active; })[0].id+'/media/insert',
												data: JSON.stringify({
													media: [API.getMedia()],
													append: true,
												}),
								...
								break;
								case 'Escape':
									var backDivSelector = '#footer-user.showing .back,'+
					...
					levelPercentage: function() {
						setTimeout(function () {
							...
							// [rawText, xpNow, xpMax]
							var numbers = $('#footer-user .bar .value')[0].innerText.match(/(\d*[,.]?\d*)(?: \/ )(\d*[,.]?\d*)/);
							if (numbers === null) { return; }

							var xp1 = parseFloat(numbers[1].replace(/[,.]/g,''));
							var xp2 = parseFloat(numbers[2].replace(/[,.]/g,''));
							var percentage = (xp1/xp2*100).toFixed(2);
					...
					popout: function() {
						setTimeout(function() {
							...
							meldBefore(popout, 'onbeforeunload', function () {
								popout = null;
							});
						}, 2000);
					},
					resize: function() {
						...
						var availWidth = Math.max(MIN_WIDTH, window.innerWidth) - CHAT_WIDTH;
						var availHeight = Math.max(MIN_HEIGHT, window.innerHeight);
						var n = availHeight - BAR_HEIGHT - 800;
						var bgTop = Math.min(BAR_HEIGHT, n);
					...
					translate: function(e) {
						if (e.target.classList.contains('translated')) {
							...
							$.ajax({
								...
								error: function(err) {
									var data = JSON.parse(err.responseText.split(",").filter(function (a) {return a.length}).join(","));
				...
				_load: function() {
					...
					$.each(require.s.contexts._.defined, function (n, obj) {
						if (!obj) { return; }
						obj._moduleID = n;
								...
								if (settings.unfriended) {
									...
									if (friendsInStorage !== null) {
										...
										if (friends.length < friendsInStorage.length) {
											var unfriends = friendsInStorage.filter(function (user) {
												var inArray = false;
													...
													if (friendsOnline[i].room.id !== storage[j].room.id &&
														...
														if (friendsOnline[i].room.slug == 'dashboard')
															...
															pi._tool.log(
																...
																e.preventDefault();
																var slug = e.data.friend.room.slug,
																    name$$1 = e.data.friend.room.name;
																modules.context.dispatch(new modules.RoomEvent(modules.RoomEvent.JOIN, slug, name$$1));
															});
														}
													}
												}
					...
					$('#pi-afk').on('click', function(e) {
						if (e.ctrlKey) {
							var afkMsg = prompt("AFK message:", settings.afkMessage);
				...
				_extendAPI: {
					init: function() {
						...
						API.getUserByName = function(name$$1) {
							if (!API.enabled) { return; }
							if (typeof name$$1 == 'undefined') {
								throw new Error('A name must be provided.');
							} else {
								var user = API.getUsers().filter(function (u) { return u.username === name$$1; });
						...
						API.addFriendByID = function(id) {
							...
							$.ajax({
								...
								success: function() {
									API.getUserByID(id, function (user) {
						...
						API.getActivePlaylist = function() {
							if (!API.enabled) { return; }
							var currentPL, name$$1, id, count, allPL;
							currentPL = $('#playlist-menu .row.selected');
							name$$1 = currentPL.children()[1].innerText;
				...
				_tool: {
					...
					getPlaylist: function(id, callback) {
						...
						var cache = _.find(require.s.contexts._.defined,function(m){return m&&m._read})._read();
						cache = JSON.parse(cache);
						var thisPL = cache[1]['p'][id]['items'];
					...
					getPlaylists: function(callback) {
						...
						$.ajax({
							...
							success: function(data) {
								...
								callback(data.data.sort(function (a,b) { return a.name.localeCompare(b.name); }));
							}
						});
					},
					...
					getRoomSettings: function() {
						...
						$.getJSON('/_/rooms/state', function(data) {
							data = data.data[0];
							var url = data.meta.description;
							var re = new RegExp('@(pi|p3|rcs)=(https?:\/\/[^\\@,;]*json)', 'ig');

							if (re.test(url)) {
								var match = url.match(re);

								var loop = function ( i ) {
									var scriptName = match[i].slice(1, match[i].indexOf("="));
								...
								for (var i = 0; i < match.length; i++) loop( i );
					...
					log: function(txt, placeType, callback) {
						var cbArgs = [], len = arguments.length - 3;
						while ( len-- > 0 ) cbArgs[ len ] = arguments[ len + 3 ];

						var type, emoticon, time;
						...
						if (placeType.indexOf('console') === -1) {
							...
							function autoHideLogs() {
								if (!document.hasFocus()) {
									...
									window.onfocus = function() {
										setTimeout(function() {
											$($logBox).fadeOut(1000, 'linear', function () { return $($logBox).remove(); });
						...
						if (placeType.indexOf('console') === -1) {
							...
							if (typeof callback === 'function') {
								...
								$logBox.on('click', function(e) {
									callback.call.apply(callback, [ methods ].concat( cbArgs ));
								});
							}
							return methods;
						}
					...
					modal: function(set, type, callback, data) {
						if (set) {
							...
							switch(type) {
								case 'custom-bg':
									var defaultURL = $('.room-background').css('background-image').split('"')[1];
									$dialog = $(
							...
							$dialog.on('click', '.dialog-frame i, .button.cancel', function () {cancel();});
							$('#dialog-container').on('click', function (e) {
								if (e.target.id !== "dialog-container") { return; }
								cancel();
							});
							$dialog.on('click', '.button.submit', function () {submit();});
							$dialog.on('submit', 'input[name="url"]', function () {submit();});
					...
					plugSocketHook: {
						...
						onConnect: function() {
							...
							this.advice = meldBefore(this.socket, 'onmessage', function (e) {
									...
									JSON.parse(e.data).forEach(function (message) {
										...
										try {
											switch (message.a) {
												...
												case 'earn':
													if (settings.gainNotification) {
														var PPEarned = message.p.pp - API.getUser().pp;
														var XPEarned = message.p.xp - API.getUser().xp;
												...
												case 'modStaff':
													message.p.u.forEach(function (e,i,a) {
														var self = API.getUser(); // Unefficient, should cache user somewhere...
					...
					showGiftDialog: function showGiftDialog(id, username, amount) {
				...
				betterClearChat: function(x) {
					var messages = $('#chat-messages > div').toArray();
				...
				soundcloudVisu: function(media) {
					...
					if (settings.scVisu && !pi._tool.getPlugSettings().streamDisabled) {
						var randomVisu = Math.floor(Math.random()*url.visu.length);
						$scFrame.hide();
						$myFrame.show();
				...
				voteAlert: function(data) {
					var userInfo = (settings.userInfo ? ' Lvl:'+data.user.level+'|'+'Id:'+data.user.id : '');
